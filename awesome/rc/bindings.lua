...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "w", function () mymainmenu:show() end,
              {description = "show main menu", group = "awesome / general"}),
  ...
  -- Run or raise applications with dmenu with elevated privileges
     ...
     end, {description = "dmenu run prompt with elevated privileges", group = "launcher"}),
    ...
    awful.key({}, "Print", function() screenshot("root") end,
        {description = "screenshot root window", group = "screenshot"}),
    awful.key({ modkey, }, "Print", screenshot,
    {description = "screenshot area (click window to screenshot it)", group = "screenshot"}),

    -- Lock Screen
    awful.key({}, "XF86ScreenSaver", function() os.execute("slock") end),
    awful.key({ modkey, }, "Delete", function() os.execute("slock") end,
      {description = "lock the screen", group = "awesome / general"}),

    -- Multimedia keys
    awful.key({ }, "XF86AudioRaiseVolume", function()
        awful.util.spawn("amixer sset -c " .. alsawidget.cardnumber .. " "  .. alsawidget.channel .. " " .. alsawidget.step .. "+")
        vicious.force({ alsawidget.bar:get_children()[1] , alsawidget.text})
        alsawidget.notify()
    end),
    ...
    awful.key({ }, "XF86AudioMute", function()
        ...
        vicious.force({ alsawidget.bar:get_children()[1], volwidget })
        alsawidget.notify()
    end),
   ...
   awful.key({ modkey }, "Tab", function ()
         mouse.screen.quakeconsole:toggle()
     end),
   awful.key({ modkey,           }, "c", function () kbdcfg.switch() end,{description = "Cycle Keyboard layout", group = "awesome / general"})
)
